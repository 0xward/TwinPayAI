      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            animate={{ scale: 1, y: 0 }}
            exit={{ scale: 0.9, y: 30 }}
            className="w-full max-w-lg bg-surface border border-white/10 rounded-3xl shadow-[0_0_60px_rgba(255,122,24,0.12)] overflow-hidden max-h-[90vh] flex flex-col"
          >
            <div className="p-6 border-b border-white/5 flex justify-between items-center ledger-strip">
              <h2 className="font-display text-xl sm:text-2xl text-gradient-brand">
              ...
              {steps.map((step, idx) => (
                <div key={idx} className="flex gap-4 items-start">
                  ...
                  <div>
                    ...
                    <p className="text-ghost text-xs mt-1">{step.desc}</p>
                  </div>
                </div>
              ))}
            </div>
            <div className="p-4 border-t border-white/5 flex items-center justify-between bg-white/5">
              ...
              <button
                onClick={onClose}
                className="px-5 py-2.5 bg-gradient-to-r from-brand-green to-brand-gold text-ink font-bold rounded-xl text-sm flex items-center gap-2 hover:scale-105 transition-transform"
              >
